...
var evmConfigSchema = exports_external.object({
  ...
  porFeedAddress: exports_external.string(),
  btcUsdFeedAddress: exports_external.string(),
  wbtcTokenAddress: exports_external.string(),
  wbtcDecimals: exports_external.number()
});
var sepoliaConfigSchema = exports_external.object({
  chainName: exports_external.string(),
  reportContractAddress: exports_external.string(),
...
var RESERVE_RATIO_GREEN = 1.02;
var RESERVE_RATIO_YELLOW = 1;
var PROOF_FRESHNESS_GREEN_HOURS = 12;
var PROOF_FRESHNESS_RED_HOURS = 24;
var CONCENTRATION_GREEN_PCT = 60;
var CONCENTRATION_RED_PCT = 75;
function readLatestRoundData(runtime2, evmClient, feedAddress) {
...
function readDecimals(runtime2, evmClient, feedAddress) {
  const callData = encodeFunctionData({
    abi: AggregatorV3,
...
function readTotalSupply(runtime2, evmClient, tokenAddress) {
  const callData = encodeFunctionData({
    abi: ERC20,
...
function fetchOnChainData(runtime2) {
  ...
  runtime2.log("Reading WBTC Proof of Reserve feed...");
  const porData = readLatestRoundData(runtime2, evmClient, evmConfig.porFeedAddress);
  const porDecimals = readDecimals(runtime2, evmClient, evmConfig.porFeedAddress);
  runtime2.log("Reading BTC/USD price feed...");
  const btcUsdData = readLatestRoundData(runtime2, evmClient, evmConfig.btcUsdFeedAddress);
  const btcUsdDecimals = readDecimals(runtime2, evmClient, evmConfig.btcUsdFeedAddress);
  runtime2.log("Reading WBTC totalSupply (liabilities)...");
  const wbtcSupply = readTotalSupply(runtime2, evmClient, evmConfig.wbtcTokenAddress);
  return {
...
function evaluateReserveRatio(reserveUsd, liabilityUsd) {
...
function evaluateProofFreshness(updatedAtUnix) {
...
var notifyBackend = (sendRequester, config, result) => {
...
function submitOnChainReport(runtime2, result) {
  const sepoliaConfig = runtime2.config.sepolia;
  const network248 = getNetwork({
    chainFamily: "evm",
  ...
  const statusCode = result.overallStatus === "GREEN" ? 0 : result.overallStatus === "YELLOW" ? 1 : 2;
  const encodedPayload = encodeAbiParameters(parseAbiParameters("uint8 status, bytes32 evidenceHash, string policyVersion, uint256 timestamp, uint8 controlCount"), [
    statusCode,
    "0x" + result.evidenceHash.substring(0, 64).padEnd(64, "0"),
  ...
  runtime2.log(`Writing report to Sepolia contract ${sepoliaConfig.reportContractAddress}...`);
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  ...
  runtime2.log(`Report anchored on Sepolia: ${txHash}`);
  runtime2.log(`View: https://sepolia.etherscan.io/tx/${txHash}`);
  return txHash;
}
var onComplianceCheck = (runtime2) => {
  runtime2.log("═══════════════════════════════════════════════════");
  runtime2.log("  CompliGuard CRE Compliance Monitor — Starting");
  runtime2.log("═══════════════════════════════════════════════════");
  runtime2.log("[Step 1/5] Reading on-chain data from Ethereum mainnet...");
  const onChainData = fetchOnChainData(runtime2);
  ...
  runtime2.log(`  PoR Reserve:  ${porBtc.toFixed(4)} BTC ($${(reserveValueUsd / 1e6).toFixed(1)}M)`);
  runtime2.log(`  WBTC Supply:  ${wbtcSupplyBtc.toFixed(4)} BTC ($${(liabilityValueUsd / 1e6).toFixed(1)}M)`);
  runtime2.log(`  BTC/USD:      $${btcUsd.toFixed(2)}`);
  runtime2.log(`  Reserve Ratio: ${reserveRatio.toFixed(4)}x`);
  runtime2.log("[Step 2/5] Evaluating compliance policy rules...");
  const controls = [
  ...
  for (const c of controls) {
    runtime2.log(`  [${c.status}] ${c.controlType}: ${c.message}`);
  }
  runtime2.log(`  Overall: ${overallStatus}`);
  runtime2.log("[Step 3/5] Generating evidence hash...");
  ...
  runtime2.log(`  Evidence: ${evidenceHash.substring(0, 16)}...`);
  ...
  runtime2.log("[Step 4/5] Notifying backend API...");
  const httpClient = new ClientCapability2;
  const notified = httpClient.sendRequest(runtime2, (sr, cfg) => notifyBackend(sr, cfg, complianceResult), consensusIdenticalAggregation())(runtime2.config).result();
  runtime2.log(`  Backend notified: ${notified}`);
  runtime2.log("[Step 5/5] Anchoring report on Sepolia...");
  const txHash = submitOnChainReport(runtime2, complianceResult);
  runtime2.log("═══════════════════════════════════════════════════");
  runtime2.log(`  CompliGuard Result: ${overallStatus}`);
  runtime2.log(`  Reserve Ratio:      ${reserveRatio.toFixed(4)}x`);
  runtime2.log(`  Evidence Hash:      ${evidenceHash.substring(0, 16)}...`);
  runtime2.log(`  Sepolia TX:         ${txHash}`);
  runtime2.log(`  Policy:             ${POLICY_VERSION}`);
  runtime2.log("═══════════════════════════════════════════════════");
...
var initWorkflow = (config) => {
  ...
  return [
    handler(cron.trigger({ schedule: config.schedule }), onComplianceCheck)
  ];
};
async function main() {
